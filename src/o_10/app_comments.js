...
function dragElement(elmnt) {
  ...
  function elementDrag(e) {
    ...
    // Commit elevator state to local storage
...
// Place all objects according to stored state information
[ 
  'planet',
  'light',
  'item',
...
// Handle "normal" button when clicked
// It posts a request to change floor together with the hmac hash 
// (authenticator) received via ajax
...
// Count particles captured in traps
...
// Kill all trapped particles older than 3 sec
const pruneCounts = () => {
...
// Target count (over 3 seconds) for each trap type
...
// Show fill level of traps through glowing wire segments, 
// Determine whether the colours are powered
// Determien which buttons are powered
const renderTraps = () => {
...
// Definition as animation callback, which gets automatically called before the next repaint of the window.
// It gets the current time as argument from the window.
function render(newtime) {
    if (stop) {
        return;
    }
    // Set this function as animation callback
    ...
    if (elapsed > fpsInterval) {
        ...
        // destroy old trapped particles
        pruneCounts();
        
        // Generate new particel at pos
        ...
        clearCanvas(ctx);
        tick++;
        // Go through all particles and determien their fate
        for (var body = world.getBodyList(); body; body = body.getNext()) {
          if (body.lifespan) {
            body.lifespan -= 1;
            let { x, y } = body.getPosition();
            // kill particles going offscreen or exceeding their lifespan
            if (!body.lifespan || x < 0 || x > CANVAS_WIDTH || y < 0 || y > CANVAS_HEIGHT) {
              ...
              // Check whether particle is in capture area of a trap.
              // If yes, destroy it and add to the particle count of the trap if colour is correct
              ...
              // Interaction with planet (gravity) objects
              ...
              // Interaction with coloured bulbs
...
// Reset button event: clear canvas after confirmation
...
// Help button event: toggle help overlay
